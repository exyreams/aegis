    ...
    <footer className="bg-muted/30 border-t border-border text-muted-foreground font-mono text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-foreground mb-4">
              ...
              />
              <span className="font-bold text-lg tracking-tight">Aegis RFQ</span>
            </div>
            <p className="text-xs leading-relaxed text-muted-foreground">
              Institutional-grade lending infrastructure powered by Canton Network. 
              Engineered for privacy, compliance, and atomic settlement.
            </p>
          ...
          {Object.entries(FOOTER_LINKS).map(([category, links]) => (
            <div key={category}>
              <h3 className="font-bold text-foreground mb-4 uppercase tracking-wider text-xs">{category}</h3>
        ...
        <div className="pt-8 border-t border-border flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-muted-foreground">
          <div>
            &copy; {new Date().getFullYear()} Aegis Financial Technologies. All rights reserved.
          </div>
