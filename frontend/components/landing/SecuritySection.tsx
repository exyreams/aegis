...
import { Shield, Lock, Server, CheckCircle2, FileCheck, Globe } from "lucide-react";

const SECURITY_LAYERS = [
  {
    title: "Application Layer",
    icon: Shield,
    features: ["Role-Based Access Control (RBAC)", "Audit Logging & Forensics", "Multi-Factor Authentication"],
    status: "SECURE",
    color: "text-primary"
  },
  {
    title: "Protocol Layer (DAML)",
    icon: FileCheck,
    features: ["Formal Verification", "Sub-Transaction Privacy", "Deterministic Execution"],
    status: "VERIFIED",
    color: "text-blue-500"
  },
  {
    ...
    icon: Server,
    features: ["Canton Ledger Nodes", "Geo-Redundant Storage", "HSM Key Management"],
    status: "HARDENED",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-foreground">
            Defense-in-Depth <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Security Stack</span>
          </h2>
          <p className="text-xl text-muted-foreground">
            Institutional-grade security baked into every layer. From the smart contract logic to the physical infrastructure.
          </p>
        </div>
            ...
            <div key={index} className="relative group">
              ...
              <div className="relative bg-card h-full p-8 rounded-xl border border-border hover:border-primary/50 transition-colors">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className={`text-xs font-mono px-2 py-1 rounded bg-muted text-foreground border border-border`}>
                    {layer.status}
                  </div>
                </div>
            ...
            {COMPLIANCE_BADGES.map((badge, index) => (
              <div key={index} className="flex flex-col items-center text-center space-y-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-mono text-muted-foreground uppercase tracking-wider mt-1">{badge.sub}</div>
                </div>
              </div>
            ))}
          </div>
