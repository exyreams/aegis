...
const MOCK_TRANSACTIONS = [
  { hash: "0x7f...3a2b", type: "RFQ_CREATE", amount: "$15,000,000", time: "2ms ago" },
  { hash: "0x9c...8d1e", type: "BID_SUBMIT", amount: "$15,000,000", time: "45ms ago" },
  { hash: "0x1a...4f5c", type: "COVENANT_CHECK", amount: "N/A", time: "120ms ago" },
  { hash: "0x3d...9e2a", type: "SETTLEMENT", amount: "$4,500,000", time: "350ms ago" },
  { hash: "0x5b...1c8f", type: "COLLATERAL_UPDATE", amount: "$12,250,000", time: "800ms ago" },
];
    ...
    const interval = setInterval(() => {
      ...
      setTransactions((prev) => {
        const newTx = {
          ...
          type: ["RFQ_CREATE", "BID_SUBMIT", "SETTLEMENT", "AUDIT_LOG"][Math.floor(Math.random() * 4)],
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            ...
            <p className="text-xl text-muted-foreground max-w-xl leading-relaxed">
              Aegis RFQ provides the cryptographic certainty and regulatory compliance required for billion-dollar credit markets. Built on DAML. Powered by Canton.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <Button variant="outline" className="h-12 px-8 border-border text-muted-foreground hover:bg-muted hover:text-foreground rounded-none font-mono uppercase tracking-wide">
                View_Documentation
              </Button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-8 pt-8 border-t border-border">
              <div>
                <div className="text-muted-foreground text-xs font-mono uppercase mb-1">Network_Volume</div>
                <div className="text-2xl font-mono font-bold text-foreground">$2.5B+</div>
              </div>
              <div>
                <div className="text-muted-foreground text-xs font-mono uppercase mb-1">Active_Nodes</div>
                <div className="text-2xl font-mono font-bold text-foreground">142</div>
              </div>
              <div>
                <div className="text-muted-foreground text-xs font-mono uppercase mb-1">Uptime_SLA</div>
                <div className="text-2xl font-mono font-bold text-primary">99.99%</div>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <div className="bg-card border border-border rounded-lg overflow-hidden shadow-2xl shadow-primary/5 font-mono text-sm">
              ...
              <div className="bg-muted/50 px-4 py-2 border-b border-border flex items-center justify-between">
                ...
                <div className="text-muted-foreground text-xs">canton-ledger-node-01</div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-muted/30 p-3 rounded border border-border">
                    <div className="text-muted-foreground text-xs mb-1 flex items-center gap-2">
                      <Server className="h-3 w-3" /> BLOCK_HEIGHT
                    </div>
                    <div className="text-primary font-bold">#{blockHeight.toLocaleString()}</div>
                  </div>
                  <div className="bg-muted/30 p-3 rounded border border-border">
                    <div className="text-muted-foreground text-xs mb-1 flex items-center gap-2">
                      <Activity className="h-3 w-3" /> NETWORK_LATENCY
                    </div>
                    <div className="text-primary font-bold">12ms</div>
                  </div>
                </div>
                ...
                <div>
                  <div className="text-muted-foreground text-xs mb-3 flex items-center gap-2">
                    <Shield className="h-3 w-3" /> LIVE_TRANSACTION_FEED
                  </div>
                  <div className="space-y-2">
                    {transactions.map((tx, i) => (
                      <div key={i} className="flex items-center justify-between text-xs p-2 hover:bg-muted/50 rounded transition-colors border-l-2 border-transparent hover:border-primary">
                        <div className="flex items-center gap-3">
                          <span className="text-muted-foreground">{tx.time}</span>
                          <span className={`font-bold ${
                            tx.type === 'RFQ_CREATE' ? 'text-blue-500' :
                            tx.type === 'BID_SUBMIT' ? 'text-indigo-500' :
                            tx.type === 'SETTLEMENT' ? 'text-primary' : 'text-foreground'
                          }`}>{tx.type}</span>
                        </div>
                        <div className="flex items-center gap-4">
                          <span className="text-muted-foreground font-mono">{tx.hash}</span>
                          <span className="text-foreground w-20 text-right">{tx.amount}</span>
                        </div>
                      </div>
                    ))}
                  </div>
