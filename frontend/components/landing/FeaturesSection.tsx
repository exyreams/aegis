"use client";

import { useState } from "react";
import { 
  Shield, 
  Lock, 
  Code, 
  Network, 
  CheckCircle2, 
  FileCode, 
  Server, 
  Database,
  ArrowRight
} from "lucide-react";
import { Button } from "@/components/ui/Button";

const DAML_SNIPPET = `template RFQ with
    borrower: Party
    rfqId: Text
    loanAmount: Decimal
    privacySettings: PrivacySettings
    covenants: [LoanCovenant]
  where
    signatory borrower
    
    -- Observer pattern for selective visibility
    observer case privacySettings.visibilityMode of
      CompletelyPrivate -> []
      SemiPublic -> lenderCriteria.preferredLenders
      PublicWithPrivateTerms -> All_Verified_Lenders

    ensure
      -- Automated compliance checks
      loanAmount > 0.0 &&
      meetsCollateralRequirements collateralAssets &&
      all (\\c -> c.status == Active) covenants`;

export function FeaturesSection() {
  const [activeTab, setActiveTab] = useState<"privacy" | "covenants" | "syndication">("privacy");

  return (
    <section id="features" className="py-24 bg-background text-foreground relative overflow-hidden">
      {/* Background Grid */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px] opacity-20" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header */}
        <div className="mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary font-mono text-xs tracking-wider mb-6">
            <Code className="h-3 w-3" />
            SYSTEM_ARCHITECTURE
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold mb-6">
            Engineered for <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Trustless Execution</span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl">
            Direct access to the underlying ledger primitives. Verify the logic before you trust the execution.
          </p>
        </div>

        {/* Feature Navigation */}
        <div className="flex flex-wrap gap-4 mb-12 border-b border-border pb-1">
          <button
            onClick={() => setActiveTab("privacy")}
            className={`px-6 py-3 text-sm font-mono transition-all border-b-2 ${
              activeTab === "privacy"
                ? "border-primary text-foreground"
                : "border-transparent text-muted-foreground hover:text-foreground"
            }`}
          >
            01_PRIVACY_ARCHITECTURE
          </button>
          <button
            onClick={() => setActiveTab("covenants")}
            className={`px-6 py-3 text-sm font-mono transition-all border-b-2 ${
              activeTab === "covenants"
                ? "border-blue-500 text-foreground"
                : "border-transparent text-muted-foreground hover:text-foreground"
            }`}
          >
            02_SMART_COVENANTS
          </button>
          <button
            onClick={() => setActiveTab("syndication")}
            className={`px-6 py-3 text-sm font-mono transition-all border-b-2 ${
              activeTab === "syndication"
                ? "border-indigo-500 text-foreground"
                : "border-transparent text-muted-foreground hover:text-foreground"
            }`}
          >
            03_SYNDICATED_NODES
          </button>
        </div>

        {/* Feature Content */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start min-h-[500px]">
          
          {/* Left: Description */}
          <div className="space-y-8 pt-4">
            {activeTab === "privacy" && (
              <div className="animate-fade-in">
                <h3 className="text-2xl font-bold mb-4 flex items-center gap-3">
                  <Shield className="h-6 w-6 text-primary" />
                  Zero-Knowledge Privacy
                </h3>
                <p className="text-muted-foreground leading-relaxed mb-6">
                  Aegis RFQ utilizes sub-transaction privacy to ensure that sensitive trade data is only visible to the direct participants of a transaction. The ledger guarantees that no unauthorized party—not even the network operator—can see your bid details.
                </p>
                <ul className="space-y-4 font-mono text-sm text-foreground/80">
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-primary" />
                    <span>Sub-transaction privacy graph</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-primary" />
                    <span>Selective disclosure proofs</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-primary" />
                    <span>Confidential counterparty discovery</span>
                  </li>
                </ul>
              </div>
            )}

            {activeTab === "covenants" && (
              <div className="animate-fade-in">
                <h3 className="text-2xl font-bold mb-4 flex items-center gap-3">
                  <FileCode className="h-6 w-6 text-blue-500" />
                  Self-Executing Covenants
                </h3>
                <p className="text-muted-foreground leading-relaxed mb-6">
                  Loan covenants are no longer static legal text. They are deterministic smart contract logic that automatically monitors compliance and triggers actions (e.g., margin calls, rate adjustments) in real-time based on oracle data.
                </p>
                <ul className="space-y-4 font-mono text-sm text-foreground/80">
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-blue-500" />
                    <span>Automated compliance monitoring</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-blue-500" />
                    <span>Deterministic default triggers</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-blue-500" />
                    <span>Real-time collateral revaluation</span>
                  </li>
                </ul>
              </div>
            )}

            {activeTab === "syndication" && (
              <div className="animate-fade-in">
                <h3 className="text-2xl font-bold mb-4 flex items-center gap-3">
                  <Network className="h-6 w-6 text-indigo-500" />
                  Atomic Syndication
                </h3>
                <p className="text-muted-foreground leading-relaxed mb-6">
                  Form lending syndicates on-the-fly. The Canton Network ensures atomic settlement across all participants, eliminating settlement risk and reducing administrative overhead to near zero.
                </p>
                <ul className="space-y-4 font-mono text-sm text-foreground/80">
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-indigo-500" />
                    <span>Multi-party atomic swaps</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-indigo-500" />
                    <span>Pro-rata payment distribution</span>
                  </li>
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-indigo-500" />
                    <span>Unified administrative view</span>
                  </li>
                </ul>
              </div>
            )}
            
            <div className="pt-8">
              <Button variant="outline" className="border-border text-muted-foreground hover:bg-muted hover:text-foreground font-mono text-xs uppercase tracking-wider">
                Read_Technical_Whitepaper <ArrowRight className="ml-2 h-3 w-3" />
              </Button>
            </div>
          </div>

          {/* Right: Visualization */}
          <div className="relative bg-card rounded-xl border border-border p-1 shadow-2xl">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-blue-500 to-indigo-500 rounded-t-xl" />
            
            {/* Code Window / Diagram Container */}
            <div className="bg-muted/30 rounded-lg overflow-hidden min-h-[400px] flex flex-col">
              {/* Window Controls */}
              <div className="flex items-center px-4 py-3 border-b border-border bg-muted/50">
                <div className="flex gap-2 mr-4">
                  <div className="w-3 h-3 rounded-full bg-destructive/50" />
                  <div className="w-3 h-3 rounded-full bg-yellow-500/50" />
                  <div className="w-3 h-3 rounded-full bg-green-500/50" />
                </div>
                <div className="text-xs font-mono text-muted-foreground">
                  {activeTab === "privacy" ? "privacy_graph.svg" : 
                   activeTab === "covenants" ? "RFQ.daml" : 
                   "network_topology.json"}
                </div>
              </div>

              {/* Content Area */}
              <div className="p-6 flex-1 relative">
                {activeTab === "privacy" && (
                  <div className="flex flex-col items-center justify-center h-full space-y-8 animate-fade-in">
                    {/* Diagram */}
                    <div className="relative w-full max-w-md">
                      <div className="flex justify-between items-center mb-12">
                        <div className="flex flex-col items-center gap-2">
                          <div className="w-12 h-12 rounded-full bg-primary/10 border border-primary flex items-center justify-center">
                            <Server className="h-6 w-6 text-primary" />
                          </div>
                          <span className="text-xs font-mono text-muted-foreground">Borrower_Node</span>
                        </div>
                        <div className="h-px flex-1 bg-gradient-to-r from-primary/50 to-blue-500/50 mx-4 relative">
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-2">
                            <Lock className="h-4 w-4 text-muted-foreground" />
                          </div>
                        </div>
                        <div className="flex flex-col items-center gap-2">
                          <div className="w-12 h-12 rounded-full bg-blue-500/10 border border-blue-500 flex items-center justify-center">
                            <Database className="h-6 w-6 text-blue-500" />
                          </div>
                          <span className="text-xs font-mono text-muted-foreground">Lender_Node</span>
                        </div>
                      </div>
                      
                      {/* Obfuscation Layer */}
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-24 bg-muted/50 border border-border rounded-lg backdrop-blur-sm flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-xs font-mono text-muted-foreground mb-1">CANTON_PRIVACY_LAYER</div>
                          <div className="flex items-center gap-2 text-xs text-primary bg-primary/10 px-2 py-1 rounded border border-primary/20">
                            <CheckCircle2 className="h-3 w-3" /> ZK_PROOF_VERIFIED
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                )}

                {activeTab === "covenants" && (
                  <div className="font-mono text-xs sm:text-sm leading-relaxed overflow-x-auto animate-fade-in">
                    <pre>
                      <code className="language-daml">
                        {DAML_SNIPPET.split('\n').map((line, i) => (
                          <div key={i} className="flex">
                            <span className="text-muted-foreground/50 mr-4 select-none w-6 text-right">{i + 1}</span>
                            <span className={
                              line.includes("template") || line.includes("where") || line.includes("ensure") ? "text-primary" :
                              line.includes("Party") || line.includes("Text") || line.includes("Decimal") ? "text-blue-500" :
                              line.includes("--") ? "text-muted-foreground italic" :
                              "text-foreground"
                            }>{line}</span>
                          </div>
                        ))}
                      </code>
                    </pre>
                  </div>
                )}

                {activeTab === "syndication" && (
                  <div className="flex items-center justify-center h-full animate-fade-in">
                    <div className="relative w-64 h-64">
                      {/* Central Node */}
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-indigo-500/20 border border-indigo-500 rounded-full flex items-center justify-center z-10">
                        <Database className="h-6 w-6 text-indigo-500" />
                      </div>
                      
                      {/* Satellite Nodes */}
                      {[0, 72, 144, 216, 288].map((deg, i) => (
                        <div 
                          key={i}
                          className="absolute top-1/2 left-1/2 w-8 h-8 bg-blue-500/20 border border-blue-500 rounded-full flex items-center justify-center"
                          style={{
                            transform: `translate(-50%, -50%) rotate(${deg}deg) translate(100px) rotate(-${deg}deg)`
                          }}
                        >
                          <Server className="h-4 w-4 text-blue-500" />
                          {/* Connection Line */}
                          <div 
                            className="absolute top-1/2 left-1/2 w-[100px] h-px bg-border -z-10 origin-left"
                            style={{
                              transform: `rotate(${deg + 180}deg)`
                            }}
                          />
                        </div>
                      ))}
                      
                      {/* Pulse Effect */}
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full border border-indigo-500/20 rounded-full animate-ping opacity-20" />
                    </div>
                  </div>
                )}
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
